...
def load_sheet_scores_from_experiment(strategy, k, task, path="./results_lower_level/lang-distance-results - final_"):
    path = path + task + ".tsv"
    with open(path, "r") as f:
        ...
        langs = []
        for line in reader:
            if line[0] == strategy and task != "ner":
                langs = [lang.split("_")[0] for lang in line[1:]]
            elif line[0] == strategy and task == "ner":
                langs = [lang for lang in line[1:]]
            elif len(langs) > 0 and int(line[0]) == k:
...
def load_average_scores_from_experiment(task="xnli", model="xlmr", strategy="k_first", k=0, measure="exact"):
    all_accs = []
    langs = get_langs_for_task(task)
    if k == 0:
        iterations = [1]
...
def compute_correlation(task, features, model, sampling_strategy, k, similarity_strategy):
    if features != "sizes":
        vecs_matrix = load_lang2vec_vectors(task, features)
        ...
        similarities = compute_similarities_of_lang_vecs(vecs_matrix, strategy=similarity_strategy)
    else:
        langs = get_langs_for_task(task)
        if model == "xlmr":
            similarities = xlmr_input_corpus_sizes(langs)
        elif model == "mbert":
            similarities = mbert_input_corpus_sizes(langs)
...
def run_correlation_analysis(task, features, model, sampling_strategy, k, aggregation_strategy, similarity_strategy):
    result = compute_correlation(task, features, model, sampling_strategy, k, similarity_strategy)
...
if __name__ == "__main__":
    for feature in l2v.FEATURE_SETS:
        ...
        # for ner its RANDOM
        result = run_correlation_analysis("pos", feature, "mbert?", "RAND", 0, "plain", "to_en")
